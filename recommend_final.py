...
# =========================================================
# STEP 6: HyperCLOVA X 최종 설명 생성
# =========================================================

DEFAULT_CLOVA_BASE_URL = (
    "https://clovastudio.stream.ntruss.com/v1/openai"
)

DEFAULT_CLOVA_MODEL = "HCX-005"
...
def _evidence_for_prompt(
    ...
    """
    LLM에는 추천에 이미 확정된 상품의 evidence만 전달한다.
    너무 긴 chunk는 비용/컨텍스트 절약을 위해 제한한다.
    """
    ...
    for ev in product.get("evidence", []):
        ...
        if (
            ...
            and len(raw_text) > max_chars_per_chunk
        ):
            raw_text = (
                raw_text[
                    :max_chars_per_chunk
...
def _source_page_summary(
    ...
    """
    페이지 표시는 LLM이 아니라 Python이 evidence metadata에서 직접 만든다.
    """
    pages: list[str] = []
...
def _build_explanation_payload(
    *,
    ...
    """
    structured DB truth + RAG evidence만 전달한다.
    """
    return {
        "user_request": user_query,
        "parsed_conditions": (
        ...
        "recommended_product": {
            ...
            "fund_name": product.get(
                "fund_name"
            ),
            "asset_manager": product.get(
                "asset_manager"
            ),
            "product_type": product.get(
                "product_type"
            ),
            "risk_grade": product.get(
                "risk_grade"
            ),
            "class_code": product.get(
                "class_code"
            ),
            "channel": product.get(
                "channel"
            ),
            "irp_explicit": product.get(
                "irp_explicit"
            ),
            "comparison_cost_pct": (
                product.get(
            ...
            "selection_reason": product.get(
                "selection_reason"
            ),
            "fee_source_page": product.get(
                "fee_source_page"
            ),
            ...
            "keyword_checks": product.get(
                "keyword_checks",
                [],
            ),
        },
        "evidence": _evidence_for_prompt(
            product
        ),
    }
...
당신은 연금·펀드 추천 결과를 설명하는 금융 AI입니다.

중요:
당신은 상품을 선택하거나 순위를 바꾸는 역할이 아닙니다.
상품 선택과 순위는 deterministic 추천 엔진이 이미 확정했습니다.
당신은 제공된 structured data와 투자설명서 evidence를
사실 그대로 설명하는 역할만 합니다.

반드시 다음 규칙을 지키세요.

1. 제공된 structured data와 evidence만 사용하세요.
2. evidence에 없는 사실을 만들지 마세요.
3. 상품명, 순위, 위험등급, 비용, class, channel, IRP 여부를 절대 변경하지 마세요.
4. 숫자를 새로 계산하거나 추정하지 마세요.
5. 다른 상품과 비교하거나 새로운 상품을 추천하지 마세요.
6. 페이지 번호를 새로 만들지 마세요. 페이지는 프로그램이 별도로 출력합니다.
7. evidence에 근거가 부족하면 정확히
   '투자설명서에서 확인되지 않음'이라고 작성하세요.
...
사용자 요청에 '안전한', '안정적인' 같은 표현이 있어도
그 표현을 상품의 객관적 특성으로 바꾸어 단정하지 마세요.

대신 반드시 검증 가능한 사실로 표현하세요.

예:
X: 위험등급이 낮아 안전성을 중시하는 사용자에게 적합합니다.
O: 사용자가 요청한 위험등급 5~6 범위에 해당합니다.

X: IRP에 적합한 상품입니다.
O: structured data에서 IRP 가입 가능 여부가 True로 확인됩니다.

X: 안전한 단기채 상품입니다.
O: 단기채 관련 투자전략이 확인되며, 사용자가 요청한 위험등급 범위에 해당합니다.

X: 원금 손실 위험이 낮습니다.
O: 투자설명서에는 원금이 보장되지 않으며 손실 가능성이 있다고 명시되어 있습니다.

추천 이유는 아래 사실만 조합해서 작성하세요.
- 사용자가 요청한 계좌 유형과의 일치 여부
- 사용자가 요청한 상품 유형과의 일치 여부
- 사용자가 요청한 위험등급 범위와의 일치 여부
- 사용자가 요청한 채널 조건과의 일치 여부
- keyword_checks에서 확인된 명시 키워드 일치 여부
- structured data에 있는 비용 정보
- evidence에서 직접 확인되는 투자대상/투자전략

주요 위험은 evidence에 명시된 내용만 사용하고,
가능하면 원금 비보장 여부를 우선적으로 언급하세요.

IRP 근거는
'IRP 명시=True', class 정보, 가입자격 evidence 등
제공된 근거를 사실 그대로 설명하세요.
'IRP에 적합하다'라고 평가하지 마세요.

출력은 정확히 아래 네 줄 형식만 사용하세요.
불필요한 인사말, 제목, 번호, 투자권유 문구는 쓰지 마세요.
...
각 항목은 1~2문장으로 짧고 이해하기 쉽게 작성하세요.
""".strip()
...
def generate_all_explanations(
    ...
    *,
    ...
    """
    한 상품씩 따로 설명한다.
    이렇게 하면 한 번의 응답에서 순서를 섞거나
    상품 간 evidence를 혼합할 가능성을 줄일 수 있다.
    """
...
def print_final_answer(
    ...
    if (
        isinstance(requested_top_k, int)
        and len(products) < requested_top_k
    ):
        print(
            f"요청은 {requested_top_k}개였지만 "
            f"조건과 근거를 모두 충족한 상품은 "
            f"{len(products)}개만 확인되어 "
            "해당 상품만 제시합니다."
        )
    ...
    for product in products:
        ...
        print(
            f"{product.get('rank')}. "
            f"{product.get('fund_name')}"
        )

        print(
            f"   - 운용사: "
            f"{product.get('asset_manager', '')}"
        )

        print(
            f"   - 위험등급: "
            f"{product.get('risk_grade', '')}"
        )

        print(
            f"   - 비용: "
            f"{product.get('comparison_cost_pct', '')}%"
            f" ({product.get('comparison_cost_basis', '')})"
        )

        print(
            f"   - 클래스/채널: "
            f"{product.get('class_code', '')} / "
            f"{product.get('channel', '')}"
        )

        print(
            f"   - IRP 명시: "
            f"{product.get('irp_explicit', '')}"
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "자연어 추천 → deterministic 추천 → 상품별 RAG → "
            "명시 키워드 검증 → HyperCLOVA X 최종 설명까지 수행합니다."
        )
    )
    ...
    parser.add_argument(
        ...
        help=(
            "최종 설명 생성 모델 "
            "(기본: CLOVA_STUDIO_MODEL 또는 HCX-005)"
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            "CLOVA Studio OpenAI 호환 base URL"
        ),
    )
    ...
    print(
        "\n[STEP 6] HyperCLOVA X 최종 설명 생성 중..."
    )
